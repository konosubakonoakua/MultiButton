...
// extern variables
double phase_ang[3] = {0};
double rms_avg[2][3];// 均值, Voltage_Deviation_Calc
double freq[3] = {0};
uint8_t flg_rms_avg;
double Data_deviation[3];
double fluctuation[3];
double flicker_Pst[3];
double flicker_Plt[3];
float HRUh[3][50];
float THDu[3];

// function prototypes
void nrf_send(uint8_t *txbuf, uint8_t size);
...
void nrf_recv_shortframe_1(void)
{
    ...
    T.vol_rms_A = (uint16_t)(rms_avg[1][0] * 100);
    T.vol_rms_B = (uint16_t)(rms_avg[1][1] * 100);
    T.vol_rms_C = (uint16_t)(rms_avg[1][2] * 100);
    T.vol_phase_BA = (uint16_t)(phase_ang[0] * 100);
    T.vol_phase_CB = (uint16_t)(phase_ang[1] * 100);
    T.vol_phase_CA = (uint16_t)(phase_ang[2] * 100);
...
void nrf_recv_shortframe_2(void)
{
    #define T vol_shortframe_2.t
    #define TAB vol_shortframe_2.t_tab
    T.bgn = VOL_SHORTFRAME_2_BGN;
    T.id = SENSOR_ID;
    T.vol_freq_deviation_A = (int16_t)((freq[0] - 50) * 100);
    T.vol_freq_deviation_B = (int16_t)((freq[1] - 50) * 100);
    T.vol_freq_deviation_C = (int16_t)((freq[2] - 50) * 100);
...
void nrf_recv_longframe_1(uint16_t buf[], uint16_t wavlen, uint16_t chl)
{
...
void nrf_recv_longframe_2(uint16_t buf[], uint16_t chl)
{
